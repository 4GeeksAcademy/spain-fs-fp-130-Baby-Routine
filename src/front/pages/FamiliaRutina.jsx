...
import { Dialog, DialogContent, DialogActions, DialogTitle, Button, Typography, TextField, MenuItem } from '@mui/material';
    ...
    return (
        <div className="bg-registro">
            ...
            <Dialog open={openEditModal} onClose={() => setOpenEditModal(false)} fullWidth maxWidth="xs" slotProps={{ paper: { style: { borderRadius: 25, padding: "10px" } } }}>
                ...
                <DialogContent>
                    <TextField label="Nombre de la actividad" fullWidth variant="outlined" margin="dense" value={editingItem?.text || ""} onChange={(e) => setEditingItem({ ...editingItem, text: e.target.value })} sx={{ mb: 2, mt: 1 }} />
                    ...
                    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="es">
                        <StaticTimePicker
                            ...
                            onChange={(newValue) => setEditingItem({ ...editingItem, time: newValue })}
                ...
                <DialogActions className="flex-column gap-2 p-4">
                    <Button fullWidth onClick={handleUpdateTask} variant="contained" style={{ backgroundColor: "var(--color-primario)", borderRadius: "12px", fontWeight: "bold", padding: "10px" }}>
                        ACTUALIZAR CAMBIOS
                    </Button>
                    <div className="d-flex w-100 gap-2">
                        <Button fullWidth onClick={() => setOpenEditModal(false)} variant="outlined" style={{ borderRadius: "10px", color: "#6c757d", borderColor: "#6c757d" }}>
                            Cerrar
                        </Button>
                        <Button fullWidth onClick={handleDeleteFromEdit} style={{ color: "#dc3545", fontWeight: "bold" }}>
            ...
            <Dialog open={openTimeModal} onClose={() => setOpenTimeModal(false)} slotProps={{ paper: { style: { borderRadius: 20 } } }}>
                ...
                <DialogActions className="justify-content-center pb-4">
                    <Button onClick={() => setOpenTimeModal(false)} variant="contained" className="w-75" style={{ backgroundColor: "var(--color-primario)", borderRadius: "12px", fontWeight: "bold" }}>
                        CONFIRMAR HORA
                    </Button>
                </DialogActions>
            </Dialog>
